...
func mustNewManagerWithTransport(t testing.TB, store Store, tr transport.Transport, opts ...ManagerOption) *Manager {
	t.Helper()
	mgr, err := NewManagerWithTransport(store, tr, opts...)
	if err != nil {
		t.Fatalf("NewManagerWithTransport failed: %v", err)
	}
	return mgr
}
